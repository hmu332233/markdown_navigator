...
var markdown_navigator = {
	...
	create_index_file(dir_paths, option) {
		...
		let index_file_data = '';
		for(var i in dir_paths) {
			let dir_path = dir_paths[i];
			const index_data = self.create_index_data(dir_path);
			index_file_data += index_data;
		}

		fs.writeFileSync(`${index_file_name}.md`, index_file_data, 'utf8');
	...
	// Returns data {string}: 한 디렉토리에 대한 index data
	//
